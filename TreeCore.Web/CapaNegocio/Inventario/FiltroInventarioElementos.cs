...
namespace TreeCore.Data
{
    ...
    public class CampoFiltroInventario
    {
        ...
        public string TipoCampo { get; set; }
        public string TypeData { get; set; }
        public string ColumnaModeloDatoID { get; set; }
    ...
    public static class CamposFiltroInventario
    {
        ...
        public static List<CampoFiltroInventario> GetCamposFiltrosInventario(long lClienteID, long InventarioCategoriaID, long lUsuarioID)
        {
            ...
            try
            {
                ...
                campoFiltroInventario = new CampoFiltroInventario
                {
                    ...
                    TipoCampo = CamposFiltroInventario.TipoCampoElemento,
                    TypeData = Comun.TIPODATO_CODIGO_LISTA,
                    QueryValores = " select EntidadID, Nombre from Entidades where Activo = 1 and ClienteID = " + lClienteID
                };
        ...
        public static List<CampoFiltroInventario> GetCamposFiltrosExportacionDatosModeloDato(long lClienteID, long ModeloDatoID, bool conAtributosDinamicos, List<long> idsCategories, bool isCategoryInventory)
        {
            string oValores;
            ...
            CampoFiltroInventario campoFiltroInventario;
            List<CoreAtributosConfiguraciones> atributosConfiguraciones;
            List<AtributosConfigWidthCategoria> atributosConfiguracionesWidthCategoria= new List<AtributosConfigWidthCategoria>(); ;
            List<CampoFiltroInventario> listaCampos = new List<CampoFiltroInventario>();
            try
            {
                #region Campos de la tabla
                List<Vw_ColumnasModeloDatos> columnas = columnasModeloDatos.GetColumnasTablasVista(ModeloDatoID);
                ...
                foreach (Vw_ColumnasModeloDatos col in columnas)
                {
                    campoFiltroInventario = new CampoFiltroInventario
                    {
                        ...
                        Campo = col.NombreColumna,
                        TipoCampo = CamposFiltroInventario.TipoCampoElemento,
                        ColumnaModeloDatoID = col.ColumnaModeloDatosID.ToString(),
                        TypeData = col.Codigo
                    };

                    if (campoFiltroInventario.TypeData == "NUMERICO")
                    {
                        listaCampos.Add(campoFiltroInventario);
                    }
                ...
                if (conAtributosDinamicos)
                {
                    if (!isCategoryInventory)
                    {
                        atributosConfiguraciones = cAtributosConf.GetActivos(lClienteID);
                    }
                    else
                    {
                        atributosConfiguracionesWidthCategoria = cCoreInventarioCategoriasAtributosCategorias.GetAtributosByInventarioCategoriasIDs(idsCategories);
                    }

                    foreach (var oAtrConfWithCat in atributosConfiguracionesWidthCategoria)
                    {

                        CoreAtributosConfiguraciones oAtrConf = oAtrConfWithCat.coreAtributoConfiguracion;
                                               
                        if (oAtrConf.TiposDatos.Codigo == "NUMERICO")
                        {
                            campoFiltroInventario = new CampoFiltroInventario
                            {
                                Name = oAtrConf.Codigo,
