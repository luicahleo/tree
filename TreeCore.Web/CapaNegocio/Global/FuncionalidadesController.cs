        ...
        public List<Vw_Funcionalidades> getListFuncionalidadesSuperUsuario(long UsuarioID)
        {
            ...
            try
            {
                ...
                if (user != null && user.EMail.Equals(Comun.TREE_SUPER_USER))
                {
                    ...
                    if (funcionalidadesTipos != null && funcionalidadesTipos.Count > 0)
                    {
                        funcionalidades = (from c in Context.Vw_Funcionalidades
                                           join f in Context.Funcionalidades on c.FuncionalidadID equals f.FuncionalidadID
                                           where f.FuncionalidadTipoID != null && funcionalidadesTipos.Contains((long)f.FuncionalidadTipoID) select c).ToList();
                    }
                }
                else
                {
