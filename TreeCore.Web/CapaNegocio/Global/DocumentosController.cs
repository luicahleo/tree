...
{
    public class DocumentosController : GeneralBaseController<Documentos, TreeCoreContext>, IGestionBasica<Documentos>
    {
        ...
        public InfoResponse Add(Documentos oEntidad) {
        ...
        public InfoResponse Update(Documentos oEntidad) {
            InfoResponse oResponse;
        ...
        public InfoResponse Delete(Documentos oEntidad) {
            InfoResponse oResponse;
                                    ...
                                    long? ProyectoID, DocumentosEstados documentoEstado, string descripcionDoc)
        {
            ...
            InfoResponse oResponse = Add(doc);

            if (oResponse != null && oResponse.Result)
            {
                doc = (Documentos)oResponse.Data;
        ...
        public bool GuardarDocumento(Ext.Net.FileUploadField file, HttpPostedFile dropzoneFile, long CreadorID, long? ProyectoTipoID,
                                    long? ObjetoID, string sObjetoTipo, long? UsuarioID, string NombreDocumento, long DocumentoTipoID,
                                    long? ProyectoID, DocumentosEstados documentoEstado, string descripcionDoc, out string errorMessage)
        {
            ...
            cHistoricoCoreDocumentos.SetDataContext(this.Context);
            cDocExt.SetDataContext(this.Context);

            List<Vw_DocumentosTiposExtensiones> lDocTiposExt;
            bool guardado = false;
            errorMessage = "";

            #region Extension No Permitida
            lDocTiposExt = cDocExt.ExtensionesPermitidasByTipoDocumento(DocumentoTipoID);
            bool extensionPermitida = ExtensionPermitida(NombreDocumento, DocumentoTipoID, lDocTiposExt);
            #endregion

            #region Nombre No Permitido
            bool nombrePermitido = NombrePermitido(NombreDocumento, true, DocumentoTipoID, ObjetoID.Value, sObjetoTipo, null);
            #endregion
            ...
            if (extensionPermitida)
            {
                if (nombrePermitido)
                {
                    ...
                    if (NombreDocumento == "" || NombreDocumento.Equals(NombreDocumento))
                    {
                        doc.Documento = getNameDocument(NombreDocumento);
                    }
                    else
                    {
                        doc.Documento = NombreDocumento;
                    ...
                    if (oResponse != null && oResponse.Result)
                    {
                        ...
                        switch (sObjetoTipo)
                        {
                            case Comun.ObjetosTipos.Seguimiento:
                                CoreWorkOrderSeguimientosDocumentosController cSegDoc = new CoreWorkOrderSeguimientosDocumentosController();
                                cSegDoc.SetDataContext(Context);
                                cSegDoc.Add(new CoreWorkOrderSeguimientosDocumentos
                                {
                                    DocumentoID = doc.DocumentoID,
                                    CoreWorkOrderSeguimientoID = (long)ObjetoID
                                });
                                break;
                            default:
                                break;
                ...
                {
                    errorMessage = GetGlobalResource("strNombreYaExisteParaElObjeto");
                }
            }
            else
            {
                errorMessage = GetGlobalResource("ExtensionNoPermitida");
            }
        ...
        public bool AddNuevaVersionDocumento(Ext.Net.FileUploadField file, HttpPostedFile dropzoneFile, string rutaDocumentoCopiaVersion, long CreadorID,
                                            long? ProyectoTipoID, long? EmplazamientoID, long? UsuarioID, long DocumentoTipoID, string NombreDocumento,
                                            long? ProyectoID, long DocumentoPadreID, long versionDocAnterior, long DocumentoEstadoID, out string errorMessage, Dictionary<string, string> messagesError)
        {
            DocumentosTiposExtensionesController cDocExt = new DocumentosTiposExtensionesController();
            cDocExt.SetDataContext(this.Context);
            ...
            using (TransactionScope trans = new TransactionScope())
            {
                try
                {
                    ...
                    string sObjetoTipo = getObjetoTipo(ultimaVersion);
                    long ObjetoID = getObjetoTipoID(ultimaVersion, sObjetoTipo);
                    ...
                    if (extensionPermitida)
                    {
                        ...
                        else if (NombreDocumento == "" || NombreDocumento.Equals(NombreDocumento))
                        {
                            doc.Documento = getNameDocument(NombreDocumento);
                        }
                        else
                    ...
                    {
                        errorMessage = messagesError["ExtensionNoPermitida"];
                    }
                }
                catch (Exception ex)
                {
            ...
            try
            {
                ...
                for (int i = 0; i < lista.Count; i++)
                {
                    if (lista[i] != nuevoDocumentoID)
                    {
                        ...
                        InfoResponse oResponse = Update(doc);
                        if (oResponse == null || !oResponse.Result)
                        {
        ...
        public bool AddNuevaVersionDocumento(Ext.Net.FileUploadField file, HttpPostedFile dropzoneFile, string rutaDocumentoCopiaVersion, long CreadorID,
                                            long? ProyectoTipoID, long? ObjetoID, string sObjetoTipo, long? UsuarioID, string NombreDocumento, long DocumentoTipoID,
                                            long? ProyectoID, long DocumentoPadreID, long versionDocAnterior, long DocumentoEstadoID,
                                            long? TamanoVersionRestaurada, Vw_Documentos docAnterior, string descripcionDoc, out Documentos doc, out string errorMessage)
        {
            bool actualizado = false;
            errorMessage = "";
            doc = null;
            ...
            try
            {
                ...
                #region Nombre No Permitido
                bool nombrePermitido = NombrePermitido(NombreDocumento, false, DocumentoTipoID, ObjetoID.Value, sObjetoTipo, DocumentoPadreID);
                #endregion
                ...
                if (extensionPermitida)
                {
                    if (nombrePermitido)
                    {
                        ...
                        try
                        {
                            ...
                            doc = setObjetoTipo(sObjetoTipo, doc, ObjetoID.Value);
                    ...
                    {
                        errorMessage = GetGlobalResource("strDocTipoExistente");
                    }
                }
                else
                {
        ...
        public InfoResponse RestaurarDocumento(long docID, long lModuloID, Usuarios usuario)
        {
            HistoricoCoreDocumentosController cHistoricoCoreDocumentos = new HistoricoCoreDocumentosController();
            this.SetDataContext(cHistoricoCoreDocumentos.Context);
            ...
            try
            {
                Vw_Documentos doc = this.GetItem<Vw_Documentos>(docID);
                Vw_CoreDocumentos coreDoc = this.GetItem<Vw_CoreDocumentos>(docID);
                if (doc != null)
                {
                    Documentos docTabla;
                    docTabla = this.GetItem(doc.DocumentoID);
                    string ruta = "";
                    long ProyectoTipoID = 0;
                    if (doc.ProyectoTipoID != null)
                    {
                        ruta = this.ObtenerRutaDocumento((long)doc.DocumentTipoID);
                        ProyectoTipoID = doc.ProyectoTipoID.Value;
                    }
                    else
                    {
                        ruta = this.ObtenerRutaDocumento((long)doc.DocumentTipoID);
                        ProyectoTipoID = lModuloID;
                    }
                    ...
                    if (File.Exists(ruta))
                    {
                        ...
                        if (doc.DocumentoPadreID != null)
                        {
                            DocumentoPadreID = doc.DocumentoPadreID.Value;
                        }
                        else
                        {
                            DocumentoPadreID = doc.DocumentoID;
                        }

                        Documentos docUltimaVersion = this.getUltimaVersion(DocumentoPadreID);

                        if (docUltimaVersion != null)
                        {

                            if (this.GetObjectDataByDocumentID(docID, out long? objetoID, out string objetoTipo))
                            {
                                Documentos docNewVersion;
                                string errorMessage = "";
                                string nameFileDoc = doc.Documento + "." + doc.Extension;
                                if (this.AddNuevaVersionDocumento(null, null, ruta, usuario.UsuarioID, ProyectoTipoID, objetoID, objetoTipo, usuario.UsuarioID, nameFileDoc, doc.DocumentTipoID.Value, doc.ProyectoID, DocumentoPadreID, docUltimaVersion.Version.Value, coreDoc.DocumentoEstadoID.Value, doc.Tamano, doc, doc.Observaciones, out docNewVersion, out errorMessage))
                                {
                                    docUltimaVersion.UltimaVersion = false;
                                    this.Update(docUltimaVersion);

                                    cHistoricoCoreDocumentos.addHistorico(docNewVersion, usuario.UsuarioID);

                                    this.SubmitChanges();

                                    infoResponse = new InfoResponse()
                                    {
                                        Data = docUltimaVersion,
                                        Description = "",
                                        Result = true
                                    };
                                }
                ...
                if (objetoID.HasValue)
                {
                    switch (objetoTipo)
                    {
                        case Comun.ObjetosTipos.Emplazamiento:
                            ...
                            cEmplazamiento.SetDataContext(this.Context);
                            Emplazamientos emp = cEmplazamiento.GetItem(objetoID.Value);
                            res = (emp == null) ? "" : emp.Codigo;
                            break;
                        case Comun.ObjetosTipos.InventarioElemento:
                            InventarioElementosController cInvElemento = new InventarioElementosController();
                            cInvElemento.SetDataContext(this.Context);
                            InventarioElementos inv = cInvElemento.GetItem(objetoID.Value);
                            res = (inv == null) ? "" : inv.NumeroInventario;
                            break;
                        case Comun.ObjetosTipos.Usuario:
                            UsuariosController cUsuarios = new UsuariosController();
                            cUsuarios.SetDataContext(this.Context);
                            Usuarios us = cUsuarios.GetItem(objetoID.Value);
                            res = (us == null) ? "" : us.EMail;
                            break;
                        default:
            ...
            try
            {
                switch (objetoTipo)
                {
                    case Comun.ObjetosTipos.Emplazamiento:
                        EmplazamientosController cEmplazamiento = new EmplazamientosController();
                        Emplazamientos emp = cEmplazamiento.GetItem(objetoID.Value);
                        res = (emp == null) ? "" : emp.NombreSitio;
                        break;
                    case Comun.ObjetosTipos.InventarioElemento:
                        InventarioElementosController cInvElemento = new InventarioElementosController();
                        InventarioElementos inv = cInvElemento.GetItem(objetoID.Value);
                        res = (inv == null) ? "" : inv.Nombre;
                        break;
                    case Comun.ObjetosTipos.Usuario:
                        UsuariosController cUsuarios = new UsuariosController();
                        Usuarios us = cUsuarios.GetItem(objetoID.Value);
                        res = (us == null) ? "" : us.NombreCompleto;
                        break;
                    default:
                        throw new Exception("GetObjectCodeByObjectID - Object type not implemented yet");
                }
        ...
        public List<Vw_CoreDocumentos> HasDocumentWhitNameAndDocumentTypeAndObjectType(string fileName, long DocumentoTipoID, long ObjetoID, string sObjetoTipo, long? padreID)
        {
            ...
            try
            {
                ...
                if (padreID.HasValue)
                {
                    query = query.Where(c => c.DocumentoID != padreID && c.DocumentoPadreID != padreID);
                }
        ...
        public bool NombrePermitido(string fileName, bool EsAgregar, long DocumentoTipoID, long ObjetoID, string sObjetoTipo, long? DocumentoPadreID)
        {
            bool nombrePermitido = true;
            try
            {
                string docName = getNameDocument(fileName);
                List<Vw_CoreDocumentos> coincidencias;

                if (EsAgregar)
                {
                    coincidencias = HasDocumentWhitNameAndDocumentTypeAndObjectType(docName, DocumentoTipoID, ObjetoID, sObjetoTipo, DocumentoPadreID);
                ...
                {
                    Documentos documentoPadre = GetItem(DocumentoPadreID.Value);
                    sObjetoTipo = getObjetoTipo(documentoPadre);
                    ObjetoID = getObjetoTipoID(documentoPadre, sObjetoTipo);

                    coincidencias = HasDocumentWhitNameAndDocumentTypeAndObjectType(docName, DocumentoTipoID, ObjetoID, sObjetoTipo, DocumentoPadreID);
                    coincidencias.ForEach(coincidencia => {
                        if (coincidencia.DocumentoID != DocumentoPadreID)
                        {
                            nombrePermitido = false;
                        }
                    });
            ...
            return nombrePermitido;
        }

        public bool ExtensionPermitida(string fileName, long DocumentoTipoID, List<Vw_DocumentosTiposExtensiones> lDocTiposExt)
        {
            bool extensionPermitida = false;

            try
            {
                string fileExtension = fileName.Split('.').ToList<string>().Last();
                foreach (Vw_DocumentosTiposExtensiones docTipExt in lDocTiposExt)
                {
                    if (String.Compare(fileExtension, docTipExt.Extension, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        // Extensión permitida
        ...
        public InfoResponse DesactivarDocumento(List<long> listDocumentoID, bool esVersion, Usuarios usuario)
        {
            HistoricoCoreDocumentosController cHistoricoCoreDocumentos = new HistoricoCoreDocumentosController();
            cHistoricoCoreDocumentos.SetDataContext(this.Context);

            InfoResponse infResponse;

            try
            {
                listDocumentoID.ForEach(DocumentoID =>
                {
                    try
                    {
                        Documentos doc = this.GetItem(DocumentoID);

                        if (doc != null)
                        {
                            if (esVersion)
                            {
                                if (doc.DocumentoPadreID.HasValue)
                                {
                                    List<Documentos> lDocVersiones = this.GetVersiones(doc.DocumentoPadreID.Value, false);

                                    //Descactivamos la versión seleccionada
                                    doc.Activo = !doc.Activo;
                                    doc.UltimaVersion = false;


                                    InfoResponse infoResponse = this.Update(doc);
                                ...
                                {
                                    //Es documento Padre, se desactiva.
                                    doc.Activo = !doc.Activo;
                                    doc.UltimaVersion = false;
                                    InfoResponse inforesponse = this.Update(doc);
                                    if (inforesponse != null && inforesponse.Result)
                            ...
                            {
                                //Es el documento General, se desactiva.
                                doc.Activo = !doc.Activo;
                                this.Update(doc);
                    ...
                    if (!esVersion)
                    {
                        Documentos histDoc = this.GetItem(DocumentoID);
                        cHistoricoCoreDocumentos.addHistorico(histDoc, usuario.UsuarioID);
                    }
                    this.SubmitChanges();
                }); 
