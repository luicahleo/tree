...
{
    public sealed class UsuariosController : GeneralBaseController<Usuarios, TreeCoreContext>, IGestionBasica<Usuarios>
    {
        ...
        public InfoResponse Add(Usuarios oUser)
        {
            InfoResponse oResponse;

            try
            {
                if (RegistroDuplicado(oUser))
                {
        ...
        public InfoResponse Update(Usuarios oUser)
        {
            InfoResponse oResponse;
        ...
        public InfoResponse Delete(Usuarios oUser)
        {
            InfoResponse oResponse;
        ...
        public bool RegistroDuplicado(Usuarios oUser)
        {
            bool isExiste = false;
            List<Usuarios> datos;

            datos = (from c in Context.Usuarios where (c.EMail == oUser.EMail && c.UsuarioID != oUser.UsuarioID) select c).ToList<Usuarios>();
        ...
        public InfoResponse AddUser(Usuarios oUser, string sAgregarUsuario)
        {
            ...
            cMonitoringModificacionesUsuarios.SetDataContext(this.Context);
            InfoResponse oResponse;
            ...
            try
            {
                ...
                if (oResponse.Result)
                {
                    MonitoringModificacionesUsuarios Historicos = new MonitoringModificacionesUsuarios();
                    Historicos.Usuarios = oUser;
                    Historicos.UsuarioModificador = oUser;
                    Historicos.FechaCreacion = oUser.FechaCreacion;
                    Historicos.FechaDesactivacion = oUser.FechaDesactivacion;
                    Historicos.OperacionRealizada = sAgregarUsuario;
                    Historicos.CambioEfectuado = sAgregarUsuario;
                    Historicos.ProyectoTipoID = Convert.ToInt32(Comun.Modulos.GLOBAL);
                    Historicos.FechaModificacion = DateTime.Now;

                    oResponse = cMonitoringModificacionesUsuarios.Add(Historicos);
                }
            }
            catch (Exception ex)
            {
        ...
        public InfoResponse AddDuplicate(long lUserID, Usuarios oUser)
        {
            MonitoringModificacionesUsuariosController cMonitoringModificacionesUsuarios = new MonitoringModificacionesUsuariosController();
            cMonitoringModificacionesUsuarios.SetDataContext(this.Context);
            ...
            cUsuariosRoles.SetDataContext(this.Context);

            UsuariosProyectosController cUsuariosProyectos = new UsuariosProyectosController();
            cUsuariosProyectos.SetDataContext(this.Context);
            ...
            try
            {
                ...
                UsuariosRoles userRoles = new UsuariosRoles();
                UsuariosProyectos userProyectos = new UsuariosProyectos();
                List<UsuariosRoles> Roles = new List<UsuariosRoles>();
                List<UsuariosProyectos> Proyectos = new List<UsuariosProyectos>();

                Roles = cUsuariosRoles.GetItemsList("UsuarioID ==" + lUserID);
                Proyectos = cUsuariosProyectos.GetItemsList("UsuarioID ==" + lUserID);

                foreach (UsuariosRoles up in Roles)
                {
                    oRoles = cRoles.GetItem(up.RolID);

                    if (oRoles.Activo == true)
                    {
                        userRoles = new UsuariosRoles();    
                        userRoles.Usuarios = oUser;
                        userRoles.RolID = up.RolID;

                        oResponse = cUsuariosRoles.Add(userRoles);
                    }
                }

                foreach (UsuariosProyectos up in Proyectos)
                {
                    userProyectos = new UsuariosProyectos();
                    userProyectos.Usuarios = oUser;
                    userProyectos.ProyectoID = up.ProyectoID;
                    userProyectos.ClienteID = up.ClienteID;
                    userProyectos.ProyectoAgrupacionID = up.ProyectoAgrupacionID;
                    userProyectos.ProyectoTipoID = up.ProyectoTipoID;
                    userProyectos.EmplazamientoZonaID = up.EmplazamientoZonaID;
                    userProyectos.PaisID = up.PaisID;
                    userProyectos.AdquisicionEmplazamientoZonaID = up.AdquisicionEmplazamientoZonaID;
                    userProyectos.CityEmplazamientoZonaID = up.CityEmplazamientoZonaID;
                    userProyectos.EnergyEmplazamientoZonaID = up.EnergyEmplazamientoZonaID;
                    userProyectos.SavingEmplazamientoZonaID = up.SavingEmplazamientoZonaID;
                    userProyectos.SharingEmplazamientoZonaID = up.SharingEmplazamientoZonaID;
                    userProyectos.UninstallAdminEmplazamientoZonaID = up.UninstallAdminEmplazamientoZonaID;
                    userProyectos.UninstallTecniaEmplazamientoZonaID = up.UninstallTecniaEmplazamientoZonaID;
                    userProyectos.AuditEmplazamientoZonaID = up.AuditEmplazamientoZonaID;
                    userProyectos.AccessEmplazamientoZonaID = up.AccessEmplazamientoZonaID;
                    userProyectos.MantenimientoEmplazamientoZonaID = up.MantenimientoEmplazamientoZonaID;
                    userProyectos.InstallTecnicaEmplazamientoZonaID = up.InstallTecnicaEmplazamientoZonaID;
                    userProyectos.LegalEmplazamientoZonaID = up.LegalEmplazamientoZonaID;
                    userProyectos.SpaceEmplazamientoZonaID = up.SpaceEmplazamientoZonaID;
                    userProyectos.SwappingEmplazamientoZonaID = up.SwappingEmplazamientoZonaID;
                    userProyectos.TorreroEmplazamientoZonaID = up.TorreroEmplazamientoZonaID;
                    userProyectos.InstallObraCivilZonaID = up.InstallObraCivilZonaID;
                    userProyectos.UninstallElectricaZonaID = up.UninstallElectricaZonaID;
                    userProyectos.AlquilerTipoContratacionID = up.AlquilerTipoContratacionID;
                    userProyectos.AlquilerTipoContratoID = up.AlquilerTipoContratoID;

                    oResponse = cUsuariosProyectos.Add(userProyectos);
                }
            }
            catch (Exception ex)
            {
        ...
        public InfoResponse AddDuplicateConfig(long lUserInicioID, long lUserDestinoID)
        {
            UsuariosRolesController cUsuariosRoles = new UsuariosRolesController();
            cUsuariosRoles.SetDataContext(this.Context);
            ...
            {
                Roles oRoles = new Roles();
                Usuarios oUserOrigen = new Usuarios();
                Usuarios oUserDestino = new Usuarios();
                UsuariosRoles oUserRoles = new UsuariosRoles();
                UsuariosProyectos oUserProyectos = new UsuariosProyectos();
                List<UsuariosRoles> listaRoles = new List<UsuariosRoles>();
                List<UsuariosProyectos> listaProyectos = new List<UsuariosProyectos>();

                List<UsuariosRoles> listaRolesDestino = new List<UsuariosRoles>();
                List<UsuariosProyectos> listaProyectosDestino = new List<UsuariosProyectos>();

                oUserOrigen = GetItem(lUserInicioID);
                oUserDestino = GetItem(lUserDestinoID);

                if (lUserInicioID != lUserDestinoID)
                {
                    listaRoles = cUsuariosRoles.GetItemsList("UsuarioID==" + oUserOrigen.UsuarioID);
                    listaProyectos = cUsuariosProyectos.GetItemsList("UsuarioID==" + oUserOrigen.UsuarioID);

                    listaRolesDestino = cUsuariosRoles.GetItemsList("UsuarioID==" + oUserDestino.UsuarioID);
                    listaProyectosDestino = cUsuariosProyectos.GetItemsList("UsuarioID==" + oUserDestino.UsuarioID);


                    foreach (UsuariosRoles t in listaRolesDestino)
                    {
                        oResponse = cUsuariosRoles.Delete(t);
                    }

                    foreach (UsuariosProyectos t in listaProyectosDestino)
                    {
                        oResponse = cUsuariosProyectos.Delete(t);
                    }

                    foreach (UsuariosRoles up in listaRoles)
                    {
                        oRoles = cRoles.GetItem(up.RolID);

                        if (oRoles.Activo == true)
                        {
                            oUserRoles = new UsuariosRoles();
                            oUserRoles.UsuarioID = oUserDestino.UsuarioID;
                            oUserRoles.RolID = up.RolID;

                            oResponse = cUsuariosRoles.Add(oUserRoles);
                        }
                    }
                    foreach (UsuariosProyectos up in listaProyectos)
                    {
                        oUserProyectos = new UsuariosProyectos();
                        oUserProyectos.UsuarioID = oUserDestino.UsuarioID;
                        oUserProyectos.ProyectoID = up.ProyectoID;
                        oUserProyectos.ClienteID = up.ClienteID;
                        oUserProyectos.ProyectoAgrupacionID = up.ProyectoAgrupacionID;
                        oUserProyectos.ProyectoTipoID = up.ProyectoTipoID;
                        oUserProyectos.EmplazamientoZonaID = up.EmplazamientoZonaID;
                        oUserProyectos.PaisID = up.PaisID;
                        oUserProyectos.AdquisicionEmplazamientoZonaID = up.AdquisicionEmplazamientoZonaID;
                        oUserProyectos.CityEmplazamientoZonaID = up.CityEmplazamientoZonaID;
                        oUserProyectos.EnergyEmplazamientoZonaID = up.EnergyEmplazamientoZonaID;
                        oUserProyectos.SavingEmplazamientoZonaID = up.SavingEmplazamientoZonaID;
                        oUserProyectos.SharingEmplazamientoZonaID = up.SharingEmplazamientoZonaID;
                        oUserProyectos.UninstallAdminEmplazamientoZonaID = up.UninstallAdminEmplazamientoZonaID;
                        oUserProyectos.UninstallTecniaEmplazamientoZonaID = up.UninstallTecniaEmplazamientoZonaID;
                        oUserProyectos.AuditEmplazamientoZonaID = up.AuditEmplazamientoZonaID;
                        oUserProyectos.AccessEmplazamientoZonaID = up.AccessEmplazamientoZonaID;
                        oUserProyectos.MantenimientoEmplazamientoZonaID = up.MantenimientoEmplazamientoZonaID;
                        oUserProyectos.InstallTecnicaEmplazamientoZonaID = up.InstallTecnicaEmplazamientoZonaID;
                        oUserProyectos.LegalEmplazamientoZonaID = up.LegalEmplazamientoZonaID;
                        oUserProyectos.SpaceEmplazamientoZonaID = up.SpaceEmplazamientoZonaID;
                        oUserProyectos.SwappingEmplazamientoZonaID = up.SwappingEmplazamientoZonaID;
                        oUserProyectos.TorreroEmplazamientoZonaID = up.TorreroEmplazamientoZonaID;
                        oUserProyectos.InstallObraCivilZonaID = up.InstallObraCivilZonaID;
                        oUserProyectos.UninstallElectricaZonaID = up.UninstallElectricaZonaID;
                        oUserProyectos.AlquilerTipoContratacionID = up.AlquilerTipoContratacionID;
                        oUserProyectos.AlquilerTipoContratoID = up.AlquilerTipoContratoID;

                        oResponse = cUsuariosProyectos.Add(oUserProyectos);
                    }
        ...
        public InfoResponse UpdateUser(Usuarios oUser, string sEditarUsuario)
        {
            MonitoringModificacionesUsuariosController cMonitoringModificacionesUsuarios = new MonitoringModificacionesUsuariosController();
            cMonitoringModificacionesUsuarios.SetDataContext(this.Context);
            InfoResponse oResponse;
            ...
            try
            {
                ...
                if (oResponse.Result)
                {
                    ...
                    Historicos.FechaCreacion = oUser.FechaCreacion;
                    Historicos.FechaDesactivacion = oUser.FechaDesactivacion;
                    Historicos.OperacionRealizada = sEditarUsuario;
                    Historicos.CambioEfectuado = sEditarUsuario;
                    Historicos.ProyectoTipoID = Convert.ToInt32(Comun.Modulos.GLOBAL);
                    Historicos.FechaModificacion = DateTime.Now;
        ...
        public InfoResponse ModificarActivar(Usuarios oUser, string sActivar, string sDesactivar)
        {
            MonitoringModificacionesUsuariosController cMonitoringModificacionesUsuarios = new MonitoringModificacionesUsuariosController();
            cMonitoringModificacionesUsuarios.SetDataContext(this.Context);
            InfoResponse oResponse;
            ...
            try
            {
                ...
                if (oResponse.Result)
                {
                    if (oUser.Activo)
                    {
                        ...
                        Historicos.FechaCreacion = oUser.FechaCreacion;
                        Historicos.FechaDesactivacion = oUser.FechaDesactivacion;
                        Historicos.OperacionRealizada = sActivar;
                        Historicos.CambioEfectuado = sActivar;
                        Historicos.ProyectoTipoID = Convert.ToInt32(Comun.Modulos.GLOBAL);
                        Historicos.FechaModificacion = DateTime.Now;
                    ...
                    else
                    {
                        ...
                        Historicos.FechaCreacion = oUser.FechaCreacion;
                        Historicos.FechaDesactivacion = oUser.FechaDesactivacion;
                        Historicos.OperacionRealizada = sDesactivar;
                        Historicos.CambioEfectuado = sDesactivar;
                        Historicos.ProyectoTipoID = Convert.ToInt32(Comun.Modulos.GLOBAL);
                        Historicos.FechaModificacion = DateTime.Now;
        ...
        public List<Usuarios> UsuariosValidosEstado(long lEstadoID, long lClienteID)
        {
            List<Usuarios> listaUsuarios;
            ...
            try
            {
                ...
                listaUsuarios = (from usu in Context.Usuarios
                                 join usuRol in Context.UsuariosRoles on usu.UsuarioID equals usuRol.UsuarioID
                                 join estRol in Context.CoreEstadosRoles on usuRol.RolID equals estRol.RolID
                                 join estSeg in Context.CoreWorkOrdersSeguimientosEstados on estRol.CoreEstadoID equals estSeg.CoreEstadoID
                                 where estSeg.CoreWorkOrderSeguimientoEstadoID == lEstadoID && usu.ClienteID == lClienteID
                                 select usu
                                 ).ToList();
