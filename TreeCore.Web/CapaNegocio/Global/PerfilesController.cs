...
namespace CapaNegocio
{
    public class PerfilesController : GeneralBaseController<Perfiles, TreeCoreContext>
    {
        ...
        public List<Perfiles> GetPerfilesByUsuario(long UsuarioID)
        {
            List<Perfiles> listaUsuarios;

            try
            {

                listaUsuarios = (from per in Context.Perfiles
                                 join perRol in Context.RolesPerfiles on per.PerfilID equals perRol.PerfilID
                                 join usuRol in Context.UsuariosRoles on perRol.RolID equals usuRol.RolID
                                 where usuRol.UsuarioID == UsuarioID
                                 select per
                                 ).ToList();
        ...
        {
            List<Perfiles> perfiles = new List<Perfiles>();
            perfiles = (from c in Context.Perfiles where c.ClienteID == lClienteID && c.Activo == true orderby c.Perfil_esES select c).ToList();
            return perfiles;
        }
