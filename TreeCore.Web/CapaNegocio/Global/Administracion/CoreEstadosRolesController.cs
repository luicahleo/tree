...
namespace CapaNegocio
{
    public class CoreEstadosRolesController : GeneralBaseController<CoreEstadosRoles, TreeCoreContext>
    {
        ...
        public List<CoreEstadosRoles> getRolByEstadoID(long lEstadoID)
        {
        ...
        public CoreEstadosRoles GetBuscado(long lEstadoID, long lRolID)
        {
            CoreEstadosRoles oRol;

            try
            {
                oRol = (from c in Context.CoreEstadosRoles where (c.CoreEstadoID == lEstadoID && c.RolID == lRolID) select c).First();
            }
            catch (Exception ex)
            {
                log.Error(ex.Message);
