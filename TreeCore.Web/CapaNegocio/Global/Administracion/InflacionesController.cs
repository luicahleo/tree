...
namespace CapaNegocio
{
    public class InflacionesController : GeneralBaseController<Inflaciones, TreeCoreContext>
    {
        ...
        public List<Inflaciones> GetAllInflacionesByClienteID(long CliID)
        {
            // Local variables
            List<Inflaciones> lista = null;
            try
            {
                lista = (from c in Context.Inflaciones where c.ClienteID == CliID select c).ToList<Inflaciones>();
            }
            catch (Exception ex)
            {
        ...
        public List<Inflaciones> GetAllInflaciones()
        {
            // Local variables
            List<Inflaciones> lista = null;
            try
            {
                lista = (from c in Context.Inflaciones select c).ToList<Inflaciones>();
            }
            catch (Exception ex)
            {
                log.Error(ex.Message);
