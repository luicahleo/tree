...
namespace CapaNegocio
{
    public class DocumentTiposController : GeneralBaseController<DocumentTipos, TreeCoreContext>
    {
        ...
        public DocumentTipos CheckExisteDocTipoMismoNombre(string nombre)
        {
            return (from c in Context.DocumentTipos where c.DocumentTipo == nombre && c.EsCarpeta select c).FirstOrDefault();
        }

        public bool CheckExisteTipoDocumentos(string nombre, long clienteID)
        {
            bool control = true;
            DocumentTipos dato = (from c in Context.DocumentTipos where c.DocumentTipo == nombre && c.ClienteID == clienteID && c.EsCarpeta == false select c).FirstOrDefault();
            if (dato == null)
            {
                control = false;
            }
