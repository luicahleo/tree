...
{
    public partial class CalidadDQGrupos : TreeCore.Page.BasePageExtNet
    {
        ...
        private List<Data.Vw_DQGroups> ListaPrincipal(int iStart, int iLimit, string sSort, string sDir, ref int iCount, string sFiltro, bool bEstado, long? lClienteID)
        {
            List<Data.Vw_DQGroups> listaDatos;
            ...
            try
            {
                if (lClienteID.HasValue)
                {
                    if (bEstado)
                    {
                        listaDatos = CDQGroups.GetItemsWithExtNetFilterList<Data.Vw_DQGroups>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "Activo == True && ClienteID == " + lClienteID);
                    }
                    else
                    {
                        listaDatos = CDQGroups.GetItemsWithExtNetFilterList<Data.Vw_DQGroups>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "ClienteID == " + lClienteID);
        ...
        public DirectResponse AgregarEditar(bool bAgregar)
        {
            ...
            try
            {
                if (!bAgregar)
                {
                    ...
                    Data.DQGroups dato = cDQGroups.GetItem(S);

                    if (dato.DQGroup == txtDQGrupo.Text)
                    {
                        dato.DQGroup = txtDQGrupo.Text;
                        dato.CreadorID = Usuario.UsuarioID;
                    ...
                    oResponse = cDQGroups.Update(dato);

                    if (oResponse.Result)
                    {
                        oResponse = cDQGroups.SubmitChanges();
                ...
                else
                {
                    ...
                    Data.DQGroups dato = new Data.DQGroups();
                    dato.DQGroup = txtDQGrupo.Text;
                    dato.CreadorID = Usuario.UsuarioID;
                    dato.Activo = true;
                    dato.FechaModificacion = DateTime.Now;
        ...
        public DirectResponse MostrarEditar()
        {
            ...
            try
            {
                ...
                Data.DQGroups dato = new Data.DQGroups();
                dato = cDQGroups.GetItem(S);
                txtDQGrupo.Text = dato.DQGroup;
        ...
        public DirectResponse Eliminar()
        {
            ...
            try
            {
                ...
                Data.DQGroups oDato = CDQGroups.GetItem(lID);
                oResponse = CDQGroups.Delete(oDato);
        ...
        {
            DirectResponse direct = new DirectResponse();
            DQGroupsController cController = new DQGroupsController();
            InfoResponse oResponse;

            try
            {
                long lID = long.Parse(GridRowSelect.SelectedRecordID);
                Data.DQGroups oDato = cController.GetItem(lID);
