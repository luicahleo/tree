...
function Grid_RowSelect(sender, registro, index) {
    ...
    if (datos != null) {
        seleccionado = datos;
        hdCodigo = seleccionado.CodigoServiceSettings;
        App.btnEditar.enable();
        App.btnAnadir.hide();
        App.btnActivar.hide();
        App.btnEliminar.hide();
        App.btnDefecto.hide();
        App.btnBloquear.hide();
    }
}
        ...
        TreeCore.MostrarEditar(
            {
                success: function (result) {
                    ...
                    App.txtHoraEjecucion.focus(false, 200);
                    App.storePrincipal.reload();
                    load.hide();
                }
            });
...
function guardarServicio(dato) {
    ...
    if (oDato.CoreProductCatalogServicioAsignadoID != undefined) {
        App.hdServicioAsignadoID.setValue(oDato.CoreProductCatalogServicioAsignadoID);
    }
    App.hdServicioID.setValue(oDato.CoreProductCatalogServicioID);
    App.hdPrecio.setValue(oDato.Precio);
    if (dato.pressed == undefined) {
        TreeCore.AgregarServicio(
            {
                success: function (result) {
                    if (result.Result != null && result.Result != '' && result.Result != 'Duplicado') {
                        Ext.Msg.alert({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.Result, buttons: Ext.Msg.OK });
                    }
                    else if (result.Result == 'Duplicado') {
                        Ext.Msg.alert({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.ErrorMessage, buttons: Ext.Msg.OK });
                    }
                    else {
