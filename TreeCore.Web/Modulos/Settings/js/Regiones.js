...
function AgregarEditar() {
    VaciarFormulario();
    App.txtRegionPais.focus(false, 200);
...
function Defecto() {
    if (registroSeleccionado(App.grid) && seleccionado != null) {
        if (App.cmbPais.selection.data.Defecto) {
            if (!seleccionado.Activo) {
                Ext.Msg.alert(
                    {
            ...
            Ext.Msg.alert({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: jsErrorDefecto, buttons: Ext.Msg.OK });
        }
    } 
}
