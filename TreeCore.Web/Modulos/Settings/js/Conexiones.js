...
function Grid_RowSelect(sender, registro, index) {
    ...
    if (datos != null) {
        ...
        if (seleccionado.ActivaConexion) {
...
function ajaxEditar() {
    ...
    if (seleccionado.IntegracionID != null) {
        App.cmbIntegracion.getTrigger(0).show();
    }
...
function SeleccionarIntegracion() {
    App.cmbIntegracion.getTrigger(0).show();
}

function RecargarIntegracion() {
    recargarCombos([App.cmbIntegracion]);
}
...
function BotonEstructura() {
    TreeCore.GenerarEstructura(
        {
            success: function (result) {
                if (result.Result != null && result.Result != '') {
                    Ext.Msg.show({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.Result, buttons: Ext.Msg.OK });
