...
{
    public partial class Idiomas : TreeCore.Page.BasePageExtNet
    {
        ...
        private List<Data.Idiomas> ListaPrincipal(int iStart, int iLimit, string sSort, string sDir, ref int iCount, string sFiltro, long? lClienteID)
        {
            List<Data.Idiomas> listaDatos;
            IdiomasController etiqNombreContoladorTabla = new IdiomasController();

            try
            {
                if (lClienteID.HasValue)
                {
                    listaDatos = etiqNombreContoladorTabla.GetItemsWithExtNetFilterList<Data.Idiomas>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "ClienteID == " + lClienteID);
                }
                else
                {
                    listaDatos = null;
        ...
        public DirectResponse AgregarEditar(bool bAgregar)
        {
...

            {
                if (!bAgregar)
                {
                    ...
                    Data.Idiomas oDato;
                    oDato = cIdiomas.GetItem(lS);
                    lCliID = long.Parse(hdCliID.Value.ToString());

                    if (cIdiomas.RegistroDuplicado(txtIdioma.Text, txtCodigoIdioma.Text, lCliID, lS))
                    {
                    ...
                    {
                        oDato.Idioma = txtIdioma.Text;
                        oDato.CodigoIdioma = txtCodigoIdioma.Text;
                    }

                    if (cIdiomas.UpdateItem(oDato))
                    {
                ...
                else
                {
                    ...
                    if (cIdiomas.RegistroDuplicado(txtIdioma.Text, txtCodigoIdioma.Text, lCliID, null))
                    {
                        log.Warn(GetGlobalResource(Comun.LogRegistroExistente));
                        MensajeBox(GetGlobalResource(Comun.jsInfo), GetGlobalResource(Comun.jsYaExiste), Ext.Net.MessageBox.Icon.INFO, null);
                    }
                    ...
                    {
                        Data.Idiomas oDato = new Data.Idiomas
                        {
                            Idioma = txtIdioma.Text,
                            CodigoIdioma = txtCodigoIdioma.Text,
                            Activo = true,
                            ClienteID = lCliID
                        };
                        if (cIdiomas.AddItem(oDato) != null)
                        {
        ...
        public DirectResponse MostrarEditar()
        {
            ...
            try
            {
                ...
                txtIdioma.Text = oDato.Idioma;
                txtCodigoIdioma.Text = oDato.CodigoIdioma;
        ...
        public DirectResponse AsignarPorDefecto()
        {
            ...
            try
            {
                ...
                oDato = cIdiomas.GetDefault(lCliID);

                // SI HAY Y ES DISTINTO AL SELECCIONADO
                if (oDato != null)
                {
                    if (oDato.IdiomaID != lID)
                    {
                        if (oDato.Defecto)
                        {
                            oDato.Defecto = !oDato.Defecto;
                            cIdiomas.UpdateItem(oDato);
                        }

                        oDato = cIdiomas.GetItem(lID);
                        oDato.Defecto = true;
                        oDato.Activo = true;
                        cIdiomas.UpdateItem(oDato);
                    }
        ...
        public DirectResponse Eliminar()
        {
            ...
            try
            {
                if (cIdiomas.RegistroDefecto(lID))
                {
                    direct.Result = GetGlobalResource(Comun.jsPorDefecto);
                    direct.Success = false;
                }
                else if (cIdiomas.DeleteItem(lID))
                {
