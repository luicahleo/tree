...
// #region INV ELEMENTOS HISTORICOS
...
function crearColumnaEstadoActualInvElemento() {

    var main = TreeCore.GetEstadoActualInvElementoJSON(
        {
...
function agregarColumnaDinamicaInvElemento(sender, registro, index) {

    TreeCore.GetEstadoActualInvElementoJSON(
        {
            ...
            success: function (result) {
                ...
                if (result.Result != null) {
                    ...
                    if (registro.id.toString() == App.hdEstadoActual.value.toString()) {
...
function agregarColumnaDinamicaInvElementoCalendario(data, event) {
...
function cargarEventosCalendario(sender, registro, index) {
    ...
    for (i = 0; i < registro.length; i++) {
        ...
        var event = {
            ...
            id: registro[i].data.InventarioElementoHistoricoID,
            date: registro[i].data.FechaModificacion,
            name: registro[i].data.FechaModificacion.toLocaleString(),
            type: 'event',

            json: JSON.stringify(registro[i].data.HistoricoAtributosDinamicos),  // CUSTOM PARAM
    ...
    $('#evoCalendar').evoCalendar({
        ...
        agregarColumnaDinamicaInvElementoCalendario(data, event);
    });
}
...
function Descargar() {
    ...
    for (var item in App.gridPrincipal.store.data.items) {
        historicos.push(App.gridPrincipal.store.data.items[item].data.InventarioElementoHistoricoID);
    }
    TreeCore.ExportarInventario(historicos, 
        {
            success: function (result) {
                if (!result.Success) {
                    Ext.Msg.alert({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.Result, buttons: Ext.Msg.OK });
