...
{
    public partial class MonitoringServiciosTree : TreeCore.Page.BasePageExtNet
    {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["opcion"] != null)
            {
                ...
                if (sOpcion == "EXPORTAR")
                {
                    try
                    {
                        List<Data.MonitoringServiciosWindows> listaDatos;
        ...
        private List<Data.MonitoringServiciosWindows> ListaPrincipal(int iStart, int iLimit, string sSort, string sDir, ref int iCount, string sFiltro, long? lClienteID, string filtroFecha = "")
        {
            List<Data.MonitoringServiciosWindows> listaDatos;
            MonitoringServiciosWindowsController CMonitoringServiciosWindows = new MonitoringServiciosWindowsController();
            string fecha;
            if (filtroFecha == "")
            {
            ...
            string queryFecha = " && " + "FechaInicio > DateTime(" + listaP[2] + "," + listaP[1] + ", " + listaP[0] + ")";
            try
            {
                if (lClienteID.HasValue)
                {
                    listaDatos = CMonitoringServiciosWindows.GetItemsWithExtNetFilterList<Data.MonitoringServiciosWindows>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "ClienteID ==" + lClienteID + queryFecha);
                }
                else
                {
                    listaDatos = null;
