...
{
    public partial class GlobalAreasOyM : TreeCore.Page.BasePageExtNet
    {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["opcion"] != null)
            {
                ...
                if (sOpcion == "EXPORTAR")
                {
                    try
                    {
                        List<Data.GlobalAreasOYM> listaDatos;
        ...
        {
            if (listaFuncionalidades.Contains((long)Comun.ModFun.ACCESO_RESTRINGIDO_A_GLOBAL_AREASOYM))
            {
            ...
            else if (listaFuncionalidades.Contains((long)Comun.ModFun.ACCESO_TOTAL_A_GLOBAL_AREASOYM))
            {
        ...
        private List<Data.GlobalAreasOYM> ListaPrincipal(int iStart, int iLimit, string sSort, string sDir, ref int iCount, string sFiltro, long? lClienteID, bool Activos)
        {
            List<Data.GlobalAreasOYM> listaDatos;
            ...
            try
            {
                if (lClienteID.HasValue && lClienteID != 0)
                {
                    if (!Activos)
                    {
                        listaDatos = CGlobalAreasOYM.GetItemsWithExtNetFilterList<Data.GlobalAreasOYM>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "ClienteID == " + lClienteID);
                    }
                    else
                    {
                        listaDatos = CGlobalAreasOYM.GetItemsWithExtNetFilterList<Data.GlobalAreasOYM>(sFiltro, sSort, sDir, iStart, iLimit, ref iCount, "ClienteID == " + lClienteID + " && Activo");
                    }
                }
                else
                {
...

        {
            ...
            GlobalAreasOYMController cAreas = new GlobalAreasOYMController();
            long cliID = 0;
            try

            {
                if (!agregar)
                {
                    ...
                    Data.GlobalAreasOYM oDato;
                    oDato = cAreas.GetItem(S);

                    if (oDato.AreaOYM == txtAreaOYM.Text)
                    {
                        oDato.AreaOYM = txtAreaOYM.Text;
                        oDato.Codigo = txtCodigo.Text;
                    }
                    else
                    {
                        cliID = long.Parse(hdCliID.Value.ToString());
                        if (cAreas.RegistroDuplicado(txtAreaOYM.Text, cliID))
                        {
                ...
                else
                {
                    ...
                    {
                        Data.GlobalAreasOYM oDato = new Data.GlobalAreasOYM();
                        oDato.AreaOYM = txtAreaOYM.Text;
                        oDato.Codigo = txtCodigo.Text;
                        oDato.Activo = true;

                        oDato.ClienteID = cliID;
                        if (cAreas.AddItem(oDato) != null)
                        {
        ...
        public DirectResponse MostrarEditar()
        {
            ...
            GlobalAreasOYMController cTipos = new GlobalAreasOYMController();
...

            {
                ...
                Data.GlobalAreasOYM oDato;
                oDato = cTipos.GetItem(S);
                txtAreaOYM.Text = oDato.AreaOYM;
        ...
        public DirectResponse AsignarPorDefecto()
        {
            ...
            try
            {
                ...
                oDato = CGlobalAreasOYM.GetDefault(long.Parse(hdCliID.Value.ToString()));

                // SI HAY Y ES DISTINTO AL SELECCIONADO
                if (oDato != null)
                {
                    if (oDato.GlobalAreaOYMID != lID)
                    {
                        if (oDato.Defecto)
                        {
                            oDato.Defecto = !oDato.Defecto;
                            CGlobalAreasOYM.UpdateItem(oDato);
                        }

                        oDato = CGlobalAreasOYM.GetItem(lID);
                        oDato.Defecto = true;
                        oDato.Activo = true;
                        CGlobalAreasOYM.UpdateItem(oDato);
                    }
        ...
        public DirectResponse Eliminar()
        {
            ...
            try
            {
                if (CGlobalAreasOYM.RegistroDefecto(lID))
                {
                    direct.Result = GetGlobalResource(Comun.jsPorDefecto);
                    direct.Success = false;
                }
                else if (CGlobalAreasOYM.DeleteItem(lID))
                {
