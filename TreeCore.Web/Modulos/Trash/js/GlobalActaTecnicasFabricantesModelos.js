...
function Grid_RowSelect(sender, registro, index) {
    ...
    if (datos != null) {
        ...
        if (seleccionado.ActivoFabricante) {
        ...
        App.hdModuloID.setValue(seleccionado.GlobalAntenaFabricanteID);
    }

    TreeCore.mostrarDetalle(seleccionado.GlobalAntenaFabricanteID);
}
...
function AgregarEditar() {
    VaciarFormulario();
    App.txtFabricante.focus(false, 500);
...
function btnCargarExcelFabricanteYmodelos() {
    ajaxCargarExcelFabricanteYmodelos();
}

function ajaxCargarExcelFabricanteYmodelos() {
    TreeCore.CargarExcelFabricanteYmodelos(
        {
            success: function (result) {
                if (result.Result != null && result.Result != '') {
                    Ext.Msg.alert({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.Result, buttons: Ext.Msg.OK });
