...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
    ...
    renderMap: function () {
        ...
        Ext.applyIf(this.mapOptions, {
            center: new google.maps.LatLng(40.336667, -3.711389)
            , zoom: 10
            , scaleControl: true
    ...
    createMarker: function (point, opt, fn) {
        ...
        if (fn != null) {
            google.maps.event.addListener(marker, "click", function (p) {
                var marks = mi.getMarkers();
                var ids = mi.getKeys();
                var index = marks.indexOf(marker);

                if (index >= 0) {
                    var cb = fn.createCallback(ids[index]);
                    cb() === fn(ids[index]);
                }
    ...
    updateMarker: function (id, point, opt, fn) {
        var index = this.keys.indexOf(id);
        if (index < 0) {
            // No existe el marcador, se inserta uno nuevo en el superposicion.
            marker = this.createMarker(point, opt, fn);
            ...
            var markerA = this.markers[index];
            markerA.setOptions(opt);
            markerA.setPosition(point);
            //this.getMap().removeOverlay(markerA);
            //this.getMap().addOverlay(marker);
            //marker.setMap(this.getMap());
            this.markers[index] = markerA;
            //google.maps.event.clearListeners(markerA, 'click');
            return markerA;
        }
    },
    ...
    onDestroy: function () {
        ...
        Ext.ux.GMapPanel.superclass.onDestroy.call(this);
    },
