...
function AgregarEditar() {
    ...
    showLoadMask(App.grid, function (load) {
        App.winGestion.setTitle(jsAgregar + ' ' + jsProyectoEstado);
        Agregar = true;
        App.winGestion.show();
        load.hide();
    });
...
function ajaxEditar() {
    VaciarFormulario();
    Agregar = false;
    App.winGestion.setTitle(jsEditar + " " + jsProyectoEstado);
...
function Eliminar() {
    if (registroSeleccionado(App.grid) && seleccionado != null) {
        Ext.Msg.alert(
            {
                title: jsEliminar + ' ' + jsProyectoEstado,
...
function Defecto() {
    if (registroSeleccionado(App.grid) && seleccionado != null) {
        if (!seleccionado.Activo) {
            Ext.Msg.alert(
                {
                    title: jsDefecto + ' ' + jsProyectoEstado,
