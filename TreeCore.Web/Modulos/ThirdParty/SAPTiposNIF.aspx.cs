...
{
    public partial class SAPTiposNIF : TreeCore.Page.BasePageExtNet
    {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            if (!IsPostBack && !RequestManager.IsAjaxRequest)
            {
                ...
                cEstadisticas.EscribeEstadisticaAccion(Usuario.UsuarioID, null, Request.Url.Segments[Request.Url.Segments.Length - 1], true, GetGlobalResource(Comun.CommentEstadisticaPageInit), "strVisitarPagina");
            ...
            if (Request.QueryString["opcion"] != null)
            {
                ...
                if (sOpcion == "EXPORTAR")
                {
                    try
                    {
                        List<TaxIdentificationNumberCategoryDTO> listaDatos = null;
                        ...
                        BaseAPIClient<TaxIdentificationNumberCategoryDTO> aPIClient = new BaseAPIClient<TaxIdentificationNumberCategoryDTO>(TOKEN_API);
                        ...
                        try
                        {
                            Comun.ExportacionDesdeListaNombreTask<TaxIdentificationNumberCategoryDTO>(grid.ColumnModel, listaDatos, Response, "", GetGlobalResource("strTipoNIF").ToString(), _Locale).Wait();
                            log.Info(GetGlobalResource(Comun.LogExcelExportado));
                            EstadisticasController cEstadisticas = new EstadisticasController();
                            cEstadisticas.registrarDescargaExcel(Usuario.UsuarioID, null, Convert.ToInt32(Comun.Modulos.GLOBAL), Request.Url.Segments[Request.Url.Segments.Length - 1], Comun.EXCEL, _Locale);
                        }
                        catch (Exception ex)
                        {
        ...
        public DirectResponse AgregarEditar(bool bAgregar)
        {
            ...
            BaseAPIClient<TaxIdentificationNumberCategoryDTO> ApiClient = new BaseAPIClient<TaxIdentificationNumberCategoryDTO>(TOKEN_API);
            TaxIdentificationNumberCategoryDTO oDato;
            ...
            try
            {
                ...
                {
                    oDato = new TaxIdentificationNumberCategoryDTO
                    {
                        Name = txtNombre.Text,
                        Code = txtCodigo.Text,
        ...
        public DirectResponse AsignarPorDefecto()
        {
            ...
            BaseAPIClient<TaxIdentificationNumberCategoryDTO> ApiClient = new BaseAPIClient<TaxIdentificationNumberCategoryDTO>(TOKEN_API);
            TaxIdentificationNumberCategoryDTO oDato;
            ResultDto<TaxIdentificationNumberCategoryDTO> oResult;
