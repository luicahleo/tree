...
{
    public partial class ProductCatalogTipos : TreeCore.Page.BasePageExtNet
    {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["opcion"] != null)
            {
                ...
                if (sOpcion == "EXPORTAR")
                {
                    try
                    {
                        List<CatalogTypeDTO> listaDatos = null;
                        ...
                        try
                        {
                            Comun.ExportacionDesdeListaNombreTask<CatalogTypeDTO>(grid.ColumnModel, listaDatos, Response, "", GetGlobalResource(Comun.jsCatalogosTipos).ToString(), _Locale).Wait();
        ...
        protected void storePrincipal_Refresh(object sender, Ext.Net.StoreReadDataEventArgs e)
        {
            if (RequestManager.IsAjaxRequest)
            {
                ...
                try
                {
                    ...
                    foreach (CatalogTypeDTO oCat in lista.Value)
                    {
                        if (oCat.IsOffering == true)
                        {
                            iValue = 1;
                        }
                        else if (oCat.IsPurchasing == true)
                        {
                            iValue = 2;
                        }

                        JsonObject oDato = new JsonObject();
                        oDato.Add("Active", oCat.Active);
                        oDato.Add("Default", oCat.Default);
                        oDato.Add("Code", oCat.Code);
                        oDato.Add("Name", oCat.Name);
                        oDato.Add("Description", oCat.Description);
                        oDato.Add("IsOffering", oCat.IsOffering);
                        oDato.Add("IsPurchasing", oCat.IsPurchasing);
                        oDato.Add("TypeBool", iValue);
                        listaFinal.Add(oDato);
                    }

                    if (listaFinal != null)
                    {
                        hdStore.SetValue(JsonConvert.SerializeObject(listaFinal));
        ...
        public DirectResponse MostrarEditar()
        {
            ...
            try
            {
                ...
                if (oDato.Value.IsOffering == true)
                {
                    isOffering.Checked = true;
                }
                else if (oDato.Value.IsPurchasing == true)
                {
                    isPurchasing.Checked = true;
                }
        ...
        public DirectResponse AsignarPorDefecto()
        {
            ...
            BaseAPIClient<CatalogTypeDTO> ApiClient = new BaseAPIClient<CatalogTypeDTO>(TOKEN_API);
            CatalogTypeDTO oDato;
            ResultDto<CatalogTypeDTO> oResult;
