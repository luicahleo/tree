...
{
    public partial class EmplazamientosGridContactos : TreeCore.Page.BasePageExtNet
    {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            if (!IsPostBack && !RequestManager.IsAjaxRequest)
            {
                ...
                Comun.Seleccionable(grdContactos, grdContactos.GetStore(), grdContactos.ColumnModel, listaIgnore, _Locale);
                ...
                if (Request.QueryString["opcion"] != null)
                {
                    ...
                    if (sOpcion == "EXPORTAR")
                    {
                        try
                        {
                            ...
                            int total = 0;
                            lista = cEmplazamientos.AplicarFiltroInterno(true, sFiltro2, -1, -1, out total, null, sFiltro, sCliente, hdStringBuscador, hdIDEmplazamientoBuscador, sResultadoKPIid, true, grdContactos.ColumnModel, sVariablesExcluidas);
                            Comun.ExportacionDesdeListaNombreTask(grdContactos.ColumnModel, lista, Response, sVariablesExcluidas, GetGlobalResource(Comun.strContactos).ToString(), Comun.DefaultLocale);
                ...
                storeContactosGlobalesEmplazamientos.Reload();
            }
        ...
        protected void storeContactosGlobalesEmplazamientos_Refresh(object sender, Ext.Net.StoreReadDataEventArgs e)
        {
            if (RequestManager.IsAjaxRequest)
            {
                try
                {
                    ...
                    lista = cEmplazamientos.AplicarFiltroInterno(true, hdFiltrosAplicados.Value.ToString(), pageSize, curPage, out total, e.Sort, s, hdCliID.Value.ToString(), hdStringBuscador, hdIDEmplazamientoBuscador, sResultadoKPIid, false, grdContactos.ColumnModel, "");
                    if (storeContactosGlobalesEmplazamientos != null && lista != null)
                    {
                        e.Total = total;
                        hdTotalCountGrid.SetValue(total);
                        storeContactosGlobalesEmplazamientos.DataSource = lista;
                        storeContactosGlobalesEmplazamientos.DataBind();
        ...
        public DirectResponse GetDatosBuscador()
        {
            ...
            try
            {
                ...
                List<JsonObject> lista;
                List<string> listaVacia = new List<string>();
                lista = cEmplazamientos.AplicarFiltroInterno(true, hdFiltrosAplicados.Value.ToString(), -1, -1, out total, null, null, hdCliID.Value.ToString(), hdStringBuscador, hdIDEmplazamientoBuscador, sResultadoKPIid, false, grdContactos.ColumnModel, "");
                direct.Result = lista;
            }
            catch (Exception ex)
            {
