...
function Grid_RowSelect(sender, registro, index) {
    ...
    if (datos != null) {
        ...
        App.btnTiposEdificios.enable();
        App.btnTiposEdificios.setTooltip(jsTiposEdificios);
...
//añadidos-----------------------------------
function BotonTiposEdificios() {
    showLoadMask(App.grid, function (load) {
        CargarStoresSerie([App.storeTiposEdificios], function (Fin) {
            if (Fin) {
                App.winTiposEdificios.show();
                load.hide();
            }
        });
    });
...
function BotonAgregarTipoEdificio() {
    showLoadMask(App.grid, function (load) {
        CargarStoresSerie([App.storeTiposEdificiosLibres], function (Fin) {
            if (Fin) {
                App.winTiposEdificiosLibres.show();
                load.hide();
            }
        });
    });
...
function Grid_RowSelectTiposEdificios(sender, registro, index) {
    var datos = registro.data;
    if (datos != null) {
        seleccionado = datos;
        App.btnQuitar.enable();
...
function DeseleccionarGrillaTiposEdificios(sender, index, registro, d) {
        App.gridTiposEdificios.clearSelections();
        App.btnQuitar.disable();
    
}

function GridTiposEdificiosLibresSeleccionar_RowSelect(sender, index ,registro) {

    var datos = registro.data;
    if (datos != null) {
        seleccionado = datos;
        btnGuardarTiposEdificiosLibre.enable();
    }
}

function BotonGuardarTiposEdificiosLibres() {
    ajaxAgregarTiposEdificios();
}

function ajaxAgregarTiposEdificios() {
    TreeCore.AgregarTiposEdificios(
        {
            success: function (result) {
                if (result.Result != null && result.Result != '') {
                    ...
                    App.winTiposEdificiosLibres.hide();
                    App.storeTiposEdificios.reload();
                }
            },
            eventMask:
            {
...
function BotonEliminarTipoEdificio() {
    if (registroSeleccionado(App.gridTiposEdificios) && seleccionado != null) {
        Ext.Msg.alert(
            {
                title: jsEliminar + ' ' + jsTituloModulo,
                msg: jsMensajeEliminar,
                buttons: Ext.Msg.YESNO,
                fn: ajaxQuitarTiposEdificios,
...
function ajaxQuitarTiposEdificios(button) {
    if (button == 'yes' || button == 'si') {
        TreeCore.QuitarTipoEdificio({
            success: function (result) {
                if (result.Result != null && result.Result != '') {
                    Ext.Msg.show({ title: jsAtencion, icon: Ext.MessageBox.WARNING, msg: result.Result, buttons: Ext.Msg.OK });
                }
