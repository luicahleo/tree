...
function seleccionarHistorico(sender, registro, index) {
    ...
    TreeCore.JSONtoDocumento(registro.data['DatosJSON'],
        {
...
function crearColumnaEstadoActualDocumentos() {

    var main = TreeCore.GetEstadoActualDocumentoJSON(
        {
...
function agregarColumnaDinamicaDocumentos(sender, registro, index) {

    TreeCore.GetEstadoActualDocumentoJSON(
        {
...
function agregarColumnaDinamicaDocumentosCalendario(data, event) {
...
function cargarEventosCalendario(sender, registro, index) {
    ...
    for (i = 0; i < registro.length; i++) {
        ...
        let objJson = JSON.parse(registro[i].data.DatosJson);


        var event = {

            id: registro[i].data.HistoricoCoreDocumentoID,
            ...
            creator: objJson.Documento // CUSTOM PARAM
        }
    ...
    $('#evoCalendar').evoCalendar({
        ...
        agregarColumnaDinamicaDocumentosCalendario(data, event);
    });
}

// #endregion

function NombreDocumentoRender(value) {
    let result = "";
    let objJson = JSON.parse(value);

    if (objJson && objJson.Documento) {
        result = objJson.Documento;
    }
