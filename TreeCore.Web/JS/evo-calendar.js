; (function (factory) {
    ...
    EvoCalendar = (function () {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.defaults = {
                ...
                firstDayOfWeek: 1,
                language: document.getElementById('hdLocale').value,
                todayHighlight: true,
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        noEventForToday: "There´s no changes today",
                        noEventForThisDay: "There´s no changes on this day", //MODIFICADO PARA HISTORICOS
                    ...
                    es: {
                        ...
                        noEventForToday: "No hay cambios hoy",
                        noEventForThisDay: "No hay cambios en este dia", //MODIFICADO PARA HISTORICOS
                        previousYearText: "Año anterior",
                        previousMonthText: "Mes anterior",
                        nextYearText: "El próximo año",
                        nextMonthText: "Mes siguiente",
                        closeSidebarText: "Cerrar la barra lateral",
                        closeEventListText: "Cerrar la lista de eventos",
                        firstHalfName: "Cambios a las "
                    },
                    de: {
            ...
            _.windowW = 290; //_.windowW = $(window).width();
            ...
            _.selectMonthBtns = $.proxy(_.selectMonthBtns, _);
    ...
    EvoCalendar.prototype.getBetweenDates = function (dates) {
        ...
        for (var x = 0; x < _.monthLength; x++) {
            var active_date = _.formatDate((_.$active.month + 1) + ' ' + (x + 1) + ' ' + _.$active.year, _.options.format);
    ...
    EvoCalendar.prototype.initEventListener = function () {
        ...
        // set event listener for month buttons (AGREGADO PARA HISTORICOS)
        _.$elements.calendarEl.find('[data-monthbtn-val]')
            .off('click.evocalendar')
            .on('click.evocalendar', _.selectMonthBtns);
    };
    ...
    EvoCalendar.prototype.buildTheBones = function () {
        ...
        // inner (MODIFICADO PARA HISTORICO)

        // BOTONES CON ICONOS
        var btnMonthPrev = '<button class="icon-button-month" type="button" data-monthbtn-val="prev" title="' + _.initials.dates[_.options.language].previousMonthText + '">' +
            '<span class="chevron-arrow-left"></span>' +
            '</button>';
        var btnMonthNext = '<button class="icon-button-month" type="button" data-monthbtn-val="next" title="' + _.initials.dates[_.options.language].nextMonthText + '">' +
            '<span class="chevron-arrow-right"></span>' +
            '</button>';
        ...
        markup += '<div class="calendar-inner">' +
            ...
            '<tr>' +
            // PREV MONTH BUTTON
            '<th colspan="1" >' + btnMonthPrev + '</th >' +
            // TITLE
            '<th colspan="5" class="text-center titleCalendar"></th >' +
            // NEXT MONTH BUTTON
            '<th colspan="1" >' + btnMonthNext + '</th >' +
            '</tr > ' +
        ...
        markup += '<div class="calendar-events">' +
            '<div class="event-header" style="display: none"><p></p></div>' +
    ...
    EvoCalendar.prototype.buildEventList = function () {
        ...
        _.$active.events = [];
        // Event date
        var numDate = (_.$active.month + 1) + "/" + _.$active.date.split('/')[0] + "/" + _.$active.year;
        var title = _.formatDate(numDate, _.options.eventHeaderFormat, _.options.language);
        ...
        if (_.options.calendarEvents) {
            for (var i = 0; i < _.options.calendarEvents.length; i++) {
                var numCalEvent = _.options.calendarEvents[i].name.split(" ")[0].split("/");
                var orderedCalEvet = numCalEvent[1] + "/" + numCalEvent[0] + "/" + numCalEvent[2];
                if (_.isBetweenDates(numDate, orderedCalEvet)) {
    ...
    EvoCalendar.prototype.addEventList = function (event_data) {
        ...
        var lastIndex = event_data.name.split(" ").length - 1;
        var eventID = event_data.name.split(" ")[lastIndex];

        var claseChecked = "x-mcombo-item-unchecked";
        if (document.getElementById("columna_historico_" + eventID) != null) {
            claseChecked = "x-mcombo-item-checked";
        }

        var eventListEl = _.$elements.eventEl.find('.event-list');
        if (eventListEl.find('[data-event-index]').length === 0) eventListEl.empty();
        _.$active.events.push(event_data);
        markup = '<div class="event-container" role="button" data-event-index="' + (event_data.id) + '" data-json=\'' + (event_data.json) + '\'>';
        markup += '<div id="eventID_' + eventID + '" class="' + claseChecked + '"><div class="event-bullet-' + event_data.type + '"';
        if (event_data.color) {
            markup += 'style="background-color:' + event_data.color + '"'
        }
        markup += '></div></div><div class="event-info"><p class="event-title">' + _.initials.dates[_.options.language].firstHalfName + _.limitTitle(eventID);
        if (event_data.badge) markup += '<span>' + event_data.badge + '</span>';
        markup += '</p>'
        markup += '<p class="event-creator">' + event_data.creator + '</p>';
    ...
    EvoCalendar.prototype.buildCalendar = function () {
        ...
        // MODIFICADO PARA HISTORICOS
        title = _.formatDate(new Date((_.$active.month + 1) + ' 1 ' + _.$active.year), _.options.titleFormat, _.options.language);

        // MODIFICADO PARA HISTORICOS
        _.$elements.innerEl.find('.calendar-table th')[1].innerText = title;
        ...
        for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
            for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
                    ...
                    var thisDay = _.formatDate((_.$active.month + 1) + ' ' + day + ' ' + _.$active.year, _.options.format);
    ...
    ****************/
    ...
    // v1.0.0 - Select month by buttons (AGREGADO PARA HISTORICOS)
    EvoCalendar.prototype.selectMonthBtns = function (event) {

        var _ = this;
        var el = $(event.target).closest('[data-monthbtn-val]');
        var monthVal = $(el).data('monthbtnVal');
        ...
        $(_.$elements.calendarEl).trigger("selectMonthBtns")
    };
