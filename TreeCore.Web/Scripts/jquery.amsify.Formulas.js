...
(function (factory) {
    ...
    (function ($, window, document, undefined) {
        ...
        AmsifySuggestags.prototype = {
            ...
            _setDefaultLabel: function (label) {
            ...
            setTagEvents: function () {
                ...
                $(this.selectors.sTagsInput).keyup(function (e) {
                    //var letra = e.key;
                    var formula = _self.selectors.sTagsInput[0].value;
                    var abre = 0;
                    var cierra = 0;
                    ...
                    if (abre == cierra) {
                        App.LbParentesis.hide();
                    } else {
                        App.LbParentesis.show();
                    }
                });
            ...
            setSuggestionsEvents: function () {
                ...
                if (this.settings.selectOnHover) {
                    $(this.selectors.listArea).find(this.classes.listItem).hover(
                        function () {
                ...
                $(this.selectors.listArea).find(this.classes.listItem).click(function () {
                    //_self.addTag($(this).text());
                    //$(_self.selectors.sTagsInput).val('').focus();
                    if (_self.selectors.sTagsInput[0].value != ' ') {
                        $(_self.selectors.sTagsInput).val(_self.selectors.sTagsInput[0].value + $(this).text());
                    } else {
            ...
            addFormula: function (value) {
                $(this.selectors.sTagsInput).val(value);
            },
            getFormula: function () {
                return this.selectors.sTagsInput[0].value;
            },
            ...
            addTag: function (value, cargador, animate = true) {
                var lista = $(this.settings.suggestions);
                if (!value) {
                    return;
                }
                // Trim value
                ...
                var html = '<span class="' + this.classes.tagItem.substring(1) + '" data-val="' + value + '">' + this.getTag(value) + ' </span> ';
                ...
                if (typeof value === "string" && value.length > this.settings.maxLenght) {
                    this.animateRemove($item, animate);
                    if (animate) {
                        this.flashItem(value);
                    }
        ...
        $.fn.amsifySuggestags = function (options, method, placeholder, listaValores) {
            return this.each(function () {
                var amsifySuggestags = new AmsifySuggestags(this);
                amsifySuggestags._settings(options);
                amsifySuggestags._setMethod(method);
                if (placeholder) {
                    amsifySuggestags._setDefaultLabel(placeholder);
                }
                amsifySuggestags._init();
                if (listaValores) {
                    for (var item in listaValores)
                        amsifySuggestags.addTag(listaValores[item]);
                }
            });
        };
