...
jQuery.fn.extend({
    expressionBuilder: function (options) {
        ...
        function getObjectVariableById(varId) {
            for (var i = 0; i < options.variables.length; i++)
                if (options.variables[i].variableId === varId)
                    return options.variables[i];
            return undefined;
        }
        function getObjectVariableByValue(varId) {
            for (var i = 0; i < options.variables.length; i++)
                if (options.variables[i].campoVinculadoID === varId)
                    return options.variables[i];
            return undefined;
        }
        ...
        var optionsHTML;
        function showSuggestions(val, cursor, mostrar=true) {
            ...
            optionsHTML = '', isFirstItem = true, items = [];
            for (var l = 0; l < options.variables.length; l++) {
                if (options.variables[l].name.toLowerCase().indexOf(varName.toLowerCase()) > -1 || options.variables[l].selectorCampoVinculado) {
                    let isSelector = false;
                    if (options.variables[l].selectorCampoVinculado) {
                        isSelector = true;
                    }
                    items.push({ id: options.variables[l].variableId, text: options.variables[l].name, selectorCampoVinculado: isSelector });
                }
            }
            var funcCount = 1;
            ...
            for (var i = 0; i < items.length; i++) {
                let myOnClick = "";
                if (items[i].selectorCampoVinculado) {
                    myOnClick = ` onclick = "openWinSelectCampoVinculado('/', ${App.hdCategoriaID.value})" `;
                }

                optionsHTML += "<div class='exp-suggestion-item " + (isFirstItem ? "selected" : "") +
                    "' data-start='" + startIndex +
                    "' data-current='" + cursor +
                    "' data-id='" + items[i].id + "' " + myOnClick + ">" + items[i].text + "</div>";
                isFirstItem = false;
            }
            if (optionsHTML != ''&& mostrar)
                suggestions.attr('data-index', 0).html(optionsHTML).slideDown();
            else
                hideSuggestions();
        }
        function selectVariable(div, tipo, aux1, aux2) {
            var selectedText;
            var text;
            if (tipo == 'texto') {
                selectedText = div.name;
                text = expressionInput.val().toString();
                tail = text.substr(aux2)
            } else {
                if (div[0].dataset.id != 0) {
                    text = expressionInput.val().toString(), start = parseInt($(div).attr('data-start')), current = parseInt($(div).attr('data-current')), tail = text.substr(current);
                    selectedText = $(div).text();
                } else {
                    return;
                }
            }
            ...
            if (tipo != 'texto') {
                ...
                if (isNumber(selectedText[0]) || hasSpecialCharacter) {
                    if (text[aux1 - 1] != '[')
                        text = text.substr(0, aux1) + "[" + selectedText + "]" + tail.trim();
                    else
                        text = text.substr(0, aux1) + selectedText + "]" + tail.trim();
                }
                else {
                    if (text[aux1 - 1] != '[')
                        ...
                        text = text.substr(0, aux1 - 1) + selectedText + tail.trim();
                }
            }
            ...
            if (tipo != 'texto') {
                ...
                for (var i = aux2; i < text.length; i++)
                    if (text[i] == "]") {
                        setCursorPosition(i + 2);
                        break;
                    }
        ...
        return {
            ...
            getObjectVariableById: function (variableId) {
                return getObjectVariableById(variableId);
            },
            getObjectVariableByValue: function (variableId) {
                return getObjectVariableByValue(variableId);
            },
            setVariables: function (vars) {
                options.variables = vars || [];
                parserOptions.variables = vars || [];
            },
            marcarCampo: function (vars, start, current) {
                selectVariable(vars, 'texto', start, current);
            },
            verVariables: function (formula, cursor) {
                showSuggestions(formula, (cursor+1), false);
            },
