...
namespace TreeCore.Page
{
    ...
    public class BasePageExtNet : System.Web.UI.Page
    {
        ...
        protected Hashtable funtionalities;
        ...
        protected override void OnPreRenderComplete(EventArgs e)
        {
            ...
            if (!IsPostBack && !RequestManager.IsAjaxRequest)
            {
                if (funtionalities != null)
                {
                    ...
                    if (listFunPag.FirstOrDefault(x => x.Split('@')[1] == "Read") == null)
                    {
                        log.Info(string.Format(Comun.MsgPaginaNoPermitida, sPagina));
                    ...
                    foreach (var Funct in funtionalities.Keys)
                    {
                        foreach (var item in (List<ComponentBase>)funtionalities[Funct]) {
                            item.Hidden = true;
                        }

                        if (listFunPag.Where(x => x.Split('@')[1] == Funct.ToString()).ToList().Count > 0)
                        {
                            foreach (var item in (List<ComponentBase>)funtionalities[Funct])
                            {
                                item.Hidden = false;
        ...
        private void Page_Init(object sender, System.EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (Session["TOKEN_API"] != null)
                    {
                        TOKEN_API = this.Session["TOKEN_API"].ToString();
                    }
                    else
                    {
                        TOKEN_API = null;
...
//#endif
        ...
        {
            ResourceManagerOperaciones(res, new List<string>(), new List<string>());
        }
        public void ResourceManagerOperaciones(ResourceManager res, List<string> pathsOfScripts, List<string> pathsOfCss)
        {
            string tema;
            ...
            Comun.AddCurrentCultureJavascript(res, this.Culture, paginaJS, aux);

            if (Header != null)
            {
                Header.InnerHtml = "";
                directorioJs = directorioJs + "/";


                List<string> pathsScripts = new List<string>();
                pathsScripts.Add(Comun.BundleConfigPaths.CONTENT_JS_GENERAL);
                pathsScripts.AddRange(pathsOfScripts);

                Header.InnerHtml += Scripts.Render(pathsScripts.ToArray());

                List<string> pathsCss = new List<string>();
                pathsCss.Add(Comun.BundleConfigPaths.CONTENT_CSS);
                pathsCss.AddRange(pathsOfCss);
                Header.InnerHtml += Styles.Render(pathsCss.ToArray());
