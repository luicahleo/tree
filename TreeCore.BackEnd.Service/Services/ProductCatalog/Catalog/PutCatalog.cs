...
{
    public class PutCatalog : PutObjectService<CatalogDTO, CatalogEntity, CatalogDTOMapper>
    {
        private readonly GetDependencies<CatalogDTO, CatalogEntity> _getCatalogDependencies;
        ...
        public PutCatalog(PutDependencies<CatalogEntity> putDependency, IHttpContextAccessor httpcontextAccessor,
            GetDependencies<CatalogDTO, CatalogEntity> getDependencies, GetDependencies<CurrencyDTO, CurrencyEntity> getCurrencyDependency, 
            GetDependencies<CatalogTypeDTO, CatalogTypeEntity> getCatalogTypeDependency, GetDependencies<InflationDTO, InflationEntity> getInflationDependency,
            GetDependencies<CatalogLifecycleStatusDTO, CatalogLifecycleStatusEntity> getLifecycleStatusDependency,
           ...
           base(httpcontextAccessor, putDependency, new CatalogValidation())
        {
            _getCatalogDependencies = getDependencies;
        ...
        public override async Task<Result<CatalogEntity>> ValidateEntity(CatalogDTO catalog, int client, string code, string email)
        {
            List<Error> lErrors = new List<Error>();
            CompanyEntity company = null;
            List<Error> lErrorsReadjustment = new List<Error>();
            Task<IEnumerable<CatalogEntity>> listCatalogs;
            ...
            CatalogEntity? prod = await _getCatalogDependencies.GetItemByCode(code, client);
            ...
            {
                catalog.CreationDate = (prod != null) ? prod.FechaCreacion : null;
                catalog.LastModificationDate = DateTime.Now;
                catalog.CreationUserEmail = (prod.UsuariosCreador != null) ? prod.UsuariosCreador.EMail : null;
                catalog.LastModificationUserEmail = (user != null) ? user.EMail : null;
            }
            ...
            if (catalog.LinkedProducts != null && catalog.LinkedProducts.Count > 0)
            {
                ...
                {
                    int i = 0;
                    foreach (ProductEntity pro in Products)
                    {
                        var linkProd = new CatalogAssignedProductsEntity(null, listaPrices[i], pro, prod);
            ...
            if (catalog.LinkedCompanies != null && catalog.LinkedCompanies.Count > 0)
            {
            ...
            CatalogEntity catalogFinal = new CatalogEntity(prod.CoreProductCatalogID, catalog.Code, catalog.Name, client, currency, catalogType, catalog.StartDate,
                catalog.EndDate, catalog.CreationDate, DateTime.Now, user, user, catalog.Description, prices, lifecycleStatus, company, linkedProducts, linkedCompanies);

            filter = new Filter(nameof(CatalogDTO.Code), Operators.eq, catalog.Code);
            listFilters.Add(filter);

            listCatalogs = _getCatalogDependencies.GetList(client, listFilters, null, null, -1, -1);
            if (listCatalogs.Result != null && listCatalogs.Result.ListOrEmpty().Count > 0 &&
                listCatalogs.Result.ListOrEmpty()[0].CoreProductCatalogID != catalogFinal.CoreProductCatalogID)
            {
