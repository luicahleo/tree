...
namespace TreeCore.BackEnd.Service.Services.Companies
{
    ...
    public class PostCompanyAssignedPaymentMethods : PostObjectService<CompanyAssignedPaymentMethodsDTO, CompanyAssignedPaymentMethodsEntity, CompanyAssignedPaymentMethodsDTOMapper>
    {
        ...
        public PostCompanyAssignedPaymentMethods(PostDependencies<CompanyAssignedPaymentMethodsEntity> postDependency,
        ...
        IHttpContextAccessor httpcontextAccessor) : base(httpcontextAccessor, postDependency, new CompanyAssignedPaymentMethodsValidation())
        {
        ...
        public override async Task<Result<CompanyAssignedPaymentMethodsEntity>> ValidateEntity(CompanyAssignedPaymentMethodsDTO paymentMethod, int clientID, string email, string code = "")
        {
            ...
            {
                lErrors.Add(Error.Create(_traduccion.CodeBankAccount + " " + $"{paymentMethod.PaymentMethodCode}" + " " + _errorTraduccion.AlreadyExist + "."));
            }
            if (lErrors.Count > 0)
            {
        ...
        public async Task<Result<List<CompanyAssignedPaymentMethodsEntity>>> ValidateEntity(List<CompanyAssignedPaymentMethodsDTO> linkedpaymentMethod, int clientID, string code = "")
        {
            List<Error> lErrors = new List<Error>();
            List<Filter> listFilters = new List<Filter>();
            Filter filter;
            ...
            foreach (CompanyAssignedPaymentMethodsDTO paymentMethodList in linkedpaymentMethod)
            {
                ...
                {
                    linkedCompanyAssignedPaymentMethodsEntity.Add(new CompanyAssignedPaymentMethodsEntity(null, null, paymentMethod, paymentMethodList.Default));
                }
